...
export async function createKanoProvider (app) {
   ...
   * @throws {ferrors.GeneralError} - If the query for the layer was limited by the service.
   */
    async compareLayers (firstElementFeatureCollection, secondElementLayer, secondElementFilter, monitor) {
      ...
      await Promise.all(firstElementFeatureCollection.features.map(async (zone) => {
        ...
        if (result.total > 0) {
          firstElementFeatures.push({ searched: zone.properties, found: result.features.map((feature) => feature.properties) })
        }
      }))
